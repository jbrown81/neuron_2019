...
node_alpha=.01; % alpha for node measures
...
for i=1:num_regions
    ...
    %end
    [strengths_pos_ttest(i,1),strengths_pos_ttest(i,2)]=ttest2(strengths_pos(i,1:num_group1),strengths_pos(i,start_group2:num_subjs),node_alpha,direction);
    [strengths_neg_ttest(i,1),strengths_neg_ttest(i,2)]=ttest2(strengths_neg(i,1:num_group1),strengths_neg(i,start_group2:num_subjs),node_alpha,direction);
    [participation_pos_ttest(i,1),participation_pos_ttest(i,2)]=ttest2(participation_pos(i,1:num_group1),participation_pos(i,start_group2:num_subjs),node_alpha,direction);
    [participation_neg_ttest(i,1),participation_neg_ttest(i,2)]=ttest2(participation_neg(i,1:num_group1),participation_neg(i,start_group2:num_subjs),node_alpha,direction);
end
...
if fdr_correct
    ...
    strengths_pos_ttest(:,1)=strengths_pos_ttest(:,2)<=fdr(strengths_pos_ttest(:,2),fdr_q_level);
    strengths_neg_ttest(:,1)=strengths_neg_ttest(:,2)<=fdr(strengths_neg_ttest(:,2),fdr_q_level);
    participation_pos_ttest(:,1)=participation_pos_ttest(:,2)<=fdr(participation_pos_ttest(:,2),fdr_q_level);
    participation_neg_ttest(:,1)=participation_neg_ttest(:,2)<=fdr(participation_neg_ttest(:,2),fdr_q_level);
end
...
disp(sprintf('unthresholded modularity Q p-value: %2.4f',Q_unthresh_p))
...
if plot_metrics == 1
    ...
    figure;
    subplot(2,2,1)
    imagesc(strengths_pos_ttest)
    title('Node unthreshold +strength')
    subplot(2,2,2)
    imagesc(strengths_neg_ttest)
    title('Node unthreshold -strength')
    subplot(2,2,3)
    imagesc(participation_pos_ttest)
    title('Node unthreshold +participation')    
    subplot(2,2,4)
    imagesc(participation_neg_ttest)
    title('Node unthreshold -participation')  
end